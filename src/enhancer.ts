...
import ora from "ora";
import { askQuestionTool, runAskQuestion, type AskQuestionArgs, readAiInstructionsTool, runReadAiInstructions, type ReadAiInstructionsArgs } from "./tools.js";
  ...
  for (let turn = 0; turn < 12; turn++) {
    ...
    for (const call of toolCalls) {
      ...
      if (call.function.name === "ask_question") {
        ...
        const answer = await runAskQuestion(parsed);
        messages.push({
          role: "tool",
          tool_call_id: call.id,
          content: answer,
        });
      } else if (call.function.name === "read_ai_instructions") {
        ...
        const result = await runReadAiInstructions(parsed);
        messages.push({
          role: "tool",
          tool_call_id: call.id,
          content: result,
